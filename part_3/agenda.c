...
char *scanString(void) {
    ...
    do {
        ...
        if (strlen(buffer) == 99) {
            printf("\nLa saisie a dépassé la limite de caractères.\n");
            continue;
        }

    } while ((sscanf(buffer, "%d", &buffer) != 1)&&((strlen(buffer) == 99)));

    vide_tampon();

    // Allouer la mémoire de la taille de notre chaîne :
...
void displayRdv(contact contact) {
    p_cell_r_d_v current = contact.racine;
